...
def main():
    ...
    while True:
        ...
        if hero.level == 10 and len(enemies) == 0 and not change_stage and ShopKeeper.just_bought:
            change_stage = True

        if change_stage:
            if s.WIDTH - (hero.player.x + hero.player.width) < 50:
                BG = pygame.transform.scale(pygame.image.load('imgs/tlo3.png'), (2000, 800))
                hero.player.x = hero.X
                hero.player.y = hero.Y
                change_stage = False
        ...
        if not change_stage:
            ...
            if len(enemies) == 0 and hero.level % 3 != 0:
                ...
                for i in range(hero.level):
                    tmp = Enemy("ziutek", s.WIDTH if random.random() < 0.5 else 0, (random.random() * ((s.HEIGHT - player.height) - (s.HEIGHT - 550))) + s.HEIGHT - 550, pygame.transform.scale(pygame.image.load('imgs/villager.png'), (60,120)), s.Scroll, 100)
                    enemies.append(tmp)
